...
struct time_stamp *g_timeshare_ptr = nullptr;
...
void LslidarC16Decoder::publishPointCloud() {
    // 使用包含时间戳的点类型
    pcl::PointCloud<PointXYZIT>::Ptr point_cloud(new pcl::PointCloud<PointXYZIT>);
    ...
    for (size_t i = 0; i < 16; ++i) {
        ...
        for (size_t j = 1; j < scan.points.size()-1; ++j) {
            ...
            point.time = static_cast<float>(scan.points[j].time);  // 添加每个点的时间戳
            point.ring = static_cast<uint16_t>(i);  // 添加线束编号
    ...
    // 写入时间戳到共享内存 (用于相机时间同步)
    if (g_timeshare_ptr != nullptr) {
        // 使用 ROS 时间戳，转换为纳秒
        int64_t timestamp_ns = static_cast<int64_t>(pc_msg.header.stamp.toNSec());
        g_timeshare_ptr->low = timestamp_ns;
    }
