...
bool LoadCovarianceDiagonal(ros::NodeHandle* nh, const std::string& param_name,
  ...
  if (values.size() != diagonal->size()) {
    ROS_WARN("%s must contain exactly 6 values, got %zu. Keeping defaults.",
...
void FillCovariance(boost::array<double, 36>& covariance,
                    const std::array<double, 6>& diagonal) {
  static const size_t kDiagonalIndices[6] = {0, 7, 14, 21, 28, 35};

  std::fill(covariance.begin(), covariance.end(), 0.0);
  for (size_t i = 0; i < diagonal.size(); ++i) {
    covariance[kDiagonalIndices[i]] = diagonal[i];
  }
}
}  // namespace
...
void RangerROSMessenger::LoadParameters() {
  ...
  LoadCovarianceDiagonal(nh_, "pose_covariance_diagonal",
                         &pose_covariance_diagonal_);
  LoadCovarianceDiagonal(nh_, "twist_covariance_diagonal",
  ...
  ROS_INFO(
      ...
      publish_odom_tf_);
  ROS_INFO(
      "Odometry covariance diagonals:\n pose  = [%g, %g, %g, %g, %g, %g]\n "
      "twist = [%g, %g, %g, %g, %g, %g]",
      pose_covariance_diagonal_[0], pose_covariance_diagonal_[1],
      pose_covariance_diagonal_[2], pose_covariance_diagonal_[3],
      pose_covariance_diagonal_[4], pose_covariance_diagonal_[5],
      twist_covariance_diagonal_[0], twist_covariance_diagonal_[1],
      twist_covariance_diagonal_[2], twist_covariance_diagonal_[3],
      twist_covariance_diagonal_[4], twist_covariance_diagonal_[5]);
...
void RangerROSMessenger::UpdateOdometry(double linear, double angular,
  ...
  FillCovariance(odom_msg.pose.covariance, pose_covariance_diagonal_);
  ...
  FillCovariance(odom_msg.twist.covariance, twist_covariance_diagonal_);
...
void RangerROSMessenger::TwistCmdCallback(
  ...
  if (parking_mode_ && robot_type_ == RangerSubType::kRangerMiniV2) {
    return;
  } else if (std::abs(msg->linear.y) > kLinearDeadband) {
    if (std::abs(msg->linear.x) <= kLinearDeadband &&
        robot_type_ == RangerSubType::kRangerMiniV1) {
      motion_mode_ = MotionState::MOTION_MODE_SIDE_SLIP;
  ...
  } else if (std::abs(msg->linear.x) <= kLinearDeadband &&
             std::abs(msg->angular.z) <= kAngularDeadband) {
  ...
  } else if (std::abs(msg->angular.z) <= kAngularDeadband) {
    motion_mode_ = MotionState::MOTION_MODE_DUAL_ACKERMAN;
    robot_->SetMotionMode(MotionState::MOTION_MODE_DUAL_ACKERMAN);
  } else {
...
double RangerROSMessenger::CalculateSteeringAngle(geometry_msgs::Twist msg,
  ...
  if (linear <= kLinearDeadband || angular <= kAngularDeadband) {
    radius = (angular <= kAngularDeadband)
                 ? robot_params_.min_turn_radius + 1.0
                 : 0.0;
    return 0.0;
  }
